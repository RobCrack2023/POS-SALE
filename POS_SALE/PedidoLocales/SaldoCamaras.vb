...
Public Class SaldoCamaras
    ...
    Public Function productosBoton(y As String)
        ...
        Dim tablas4 As DataTable = New DataTable()
        ...
        For g = 0 To tablas.Rows.Count - 1
            ...
            For i = 0 To tablas2.Rows.Count - 1
                ...
                tablas3.Reset()
                sql = "select count(b.cantidad) as total, sum(b.cantidad) as cantidad "
                sql = sql & " from pedidolocal_cab a "
                sql = sql & " inner join pedidolocal_det b ON a.`idpedido_cab`=b.`idpedido_cab`"
                sql = sql & " where  b.id_producto=" & tablas2.Rows(i)("codarticulo") & " and "
                sql = sql & "  a.fecha_pedido='" & Format(CDate(txtfechaped.Text), "yyyy-MM-dd") & "'"
                tablas3.Load(objconnn.executarmysql(sql))




                If tablas3.Rows(0)("total") > 0 Then

                    tablas4.Reset()

                    sql = " select pedido_total,"
                    sql = sql & "inicial,"
                    sql = sql & "descontar1,"
                    sql = sql & "descontar2,"
                    sql = sql & "sugerida,"
                    ...
                    sql = sql & "fin  from pedido_detsaldo where  id_cabsaldo=999999  and id_producto=" & tablas2.Rows(i)("codarticulo")

                    tablas4.Load(objconnn.executarmysql(sql))


                    '-------------------------------------------------------------


                    If tablas4.Rows.Count > 0 Then
                        grillaprodsol.Rows.Add(tablas2.Rows(i)("codarticulo"), tablas2.Rows(i)("descripcion").ToString(), tablas3.Rows(0)("cantidad"), tablas4.Rows(0)("inicial"), tablas4.Rows(0)("descontar1"), tablas4.Rows(0)("descontar2"), tablas4.Rows(0)("fin"), 0, tablas4.Rows(0)("sugerida"), tablas4.Rows(0)("produccion"))
                    Else
                        grillaprodsol.Rows.Add(tablas2.Rows(i)("codarticulo"), tablas2.Rows(i)("descripcion").ToString(), tablas3.Rows(0)("cantidad"), 0, 0, 0, 0, 0, 0, 0)
                    End If
                    grillaprodsol.Rows.Item(grillaprodsol.Rows.GetLastRow(DataGridViewElementStates.None)).DefaultCellStyle.BackColor = Color.Chocolate
                ...
                Else
                    ...
                    If tablas4.Rows.Count > 0 Then
                        grillaprodsol.Rows.Add(tablas2.Rows(i)("codarticulo"), tablas2.Rows(i)("descripcion").ToString(), tablas3.Rows(0)("cantidad"), tablas4.Rows(0)("inicial"), tablas4.Rows(0)("descontar1"), tablas4.Rows(0)("descontar2"), tablas4.Rows(0)("fin"), 0, tablas4.Rows(0)("sugerida"), tablas4.Rows(0)("produccion"))
                        grillaprodsol.Rows.Item(grillaprodsol.Rows.GetLastRow(DataGridViewElementStates.None)).DefaultCellStyle.BackColor = Color.Coral
                        grillaprodsol.Rows.Item(grillaprodsol.Rows.GetLastRow(DataGridViewElementStates.None)).DefaultCellStyle.ForeColor = Color.White

                    Else
                        grillaprodsol.Rows.Add(tablas2.Rows(i)("codarticulo"), tablas2.Rows(i)("descripcion").ToString(), 0, 0, 0, 0, 0, 0, 0, 0)

                        grillaprodsol.Rows.Item(grillaprodsol.Rows.GetLastRow(DataGridViewElementStates.None)).DefaultCellStyle.BackColor = Color.White

                        grillaprodsol.Rows.Item(grillaprodsol.Rows.GetLastRow(DataGridViewElementStates.None)).DefaultCellStyle.ForeColor = Color.Black
                    End If
    ...
    Private Sub grillaprodsol_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles grillaprodsol.CellValueChanged
        ...
        If w > -1 Then
            ...
            If grillaprodsol.Item(3, w).Value < 1 And grillaprodsol.Item(4, w).Value < 1 And grillaprodsol.Item(5, w).Value < 1 Then
                ...
                subtotal = Val(grillaprodsol.Item(3, w).Value) - (Val(grillaprodsol.Item(4, w).Value + Val(grillaprodsol.Item(5, w).Value)))
                grillaprodsol.Item(6, w).Value = subtotal

                totalcantprod()

                eliminaproductos(999999, grillaprodsol.Item(0, w).Value)
            ...
            Else
                ...
                agregaproductos(999999, grillaprodsol.Item(0, w).Value, grillaprodsol.Item(2, w).Value, grillaprodsol.Item(3, w).Value, grillaprodsol.Item(4, w).Value, grillaprodsol.Item(5, w).Value, grillaprodsol.Item(8, w).Value, grillaprodsol.Item(6, w).Value, grillaprodsol.Item(9, w).Value)
    ...
    End Sub
    Sub agregaproductos(idcab As Integer, idproducto As Integer, total As Integer, inicial As Integer, descontar1 As Integer, descontar2 As Integer, sugerida As Integer, fin As Integer, produccion As Integer)
        Dim sql As String
        Dim objconnn As DBCONECTAR = New DBCONECTAR
        Dim tablas As DataTable = New DataTable()

        tablas.Reset()
        sql = "select id_producto from pedido_detsaldo where id_cabsaldo=" & idcab & " and id_producto=" & idproducto

        tablas.Load(objconnn.executarmysql(sql))

        If Val(tablas.Rows.Count) > 0 Then

            sql = "update pedido_detsaldo  set  pedido_total=" & total & ",inicial=" & inicial & ",descontar1=" & descontar1 & ",descontar2=" & descontar2 & ",sugerida=" & sugerida & ",fin=" & fin & ",produccion=" & produccion & " where  id_cabsaldo=" & idcab & " and   id_producto=" & idproducto
            objconnn.executarmysqlinsert(sql)
        Else
            sql = "insert into pedido_detsaldo (id_cabsaldo,pedido_total,id_producto,inicial,descontar1,descontar2,sugerida,fin)  values (" & idcab & "," & total & "," & idproducto & "," & inicial & "," & descontar1 & "," & descontar2 & "," & sugerida & "," & fin & ")"
            objconnn.executarmysqlinsert(sql)
        End If

    End Sub
    Sub eliminaproductos(idcab As Integer, idproducto As Integer)
        Dim sql As String
        Dim objconnn As DBCONECTAR = New DBCONECTAR
        Dim tablas As DataTable = New DataTable()
        ...
        If Val(tablas.Rows.Count) > 0 Then
            ...
            sql = "delete   from  pedido_saldo   where  id_cabsaldo=" & idcab & " and   id_producto=" & idproducto
            objconnn.executarmysqlinsert(sql)
    ...
    Sub EliminaPedidosTmp()

        Dim sql As String
        Dim objconnn As DBCONECTAR = New DBCONECTAR

        sql = "delete from pedido_detsaldo where id_cabsaldo=999999"
        objconnn.executarmysqlinsert(sql)
    ...
    Private Sub btnenviar_Click(sender As Object, e As EventArgs) Handles btnenviar.Click
        ...
        sql = "select idpedido_det from pedido_detsaldo where idpedido_cab=999999"
        tablas.Load(objconnn.executarmysql(sql))
        ...
        'sql = "INSERT INTO pedido_cabsaldo "
        'sql = sql & " (`id_pedidocabsaldo`, `fec_saldo`,`hrs_saldo`,`usr`) "
    ...
    Private Sub btngrabarsaldo_Click(sender As Object, e As EventArgs) Handles btngrabarsaldo.Click
